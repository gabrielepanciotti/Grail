...
for method, reducer in reduction_methods.items():
    ...
    if method == "Point Cloud":
        ...
        reduced_test, reduction_time_test, compression_ratio_test, _ = reducer(test_data.values, pca_model=pca_model)
        reduced_label_train, reduced_label_test = train_labels, test_labels
    else:  # Clustering
...
reduction_results_df.to_csv("reduction_results.csv", index=False)